...
class CareerPipeline:
    ...
    def run_discovery(
        ...
        ctx.record_step("CareerOrchestrator", "discovery complete", elapsed_s=round(time.time() - t0, 3))
    ...
    def run_prepare(
        ...
        """Phase 2: Resume -> Cover Letter -> Apply -> Skills for user-selected jobs only."""
        ...
        ranked = orch.matching.run(profile, target_jobs, top_k=len(target_jobs))
        ...
        ctx.record_step("CareerOrchestrator", "preparation complete", elapsed_s=round(time.time() - t0, 3))
